...
function base16_encode(bit_array2) {
  let result = "";
  for (const byte of bit_array2.buffer) {
...
// build/dev/javascript/python_editor/python_editor.ffi.mjs
function do_run_code(code2, on_output) {
  Sk.configure({
    output: on_output,
...
async function sha256_hash(value3) {
  const encoder = new TextEncoder();
  const data = encoder.encode(value3);
  return window.crypto.subtle.digest("SHA-256", data).then((buffer) => toBitArray(new Uint8Array(buffer)));
}
function save_to_session_storage(key, value3) {
  sessionStorage.setItem(key, value3);
}
function read_from_session_storage(key) {
  return sessionStorage.getItem(key) ?? "";
}

// build/dev/javascript/python_editor/python_editor.mjs
var Model2 = class extends CustomType {
  constructor(code2, output, outcome, times_edited, times_ran) {
    super();
    this.code = code2;
    this.output = output;
    this.outcome = outcome;
    this.times_edited = times_edited;
...
var FromPreviousRun = class extends CustomType {
  constructor(output) {
    super();
    this.output = output;
  }
};
var FromCurrentRun = class extends CustomType {
  constructor(output) {
    super();
    this.output = output;
  }
};
var UserTypedCode = class extends CustomType {
  constructor(code2) {
    super();
    this.code = code2;
  }
};
var UserPressedRunButton = class extends CustomType {
};
var UserPressedSaveButton = class extends CustomType {
};
var SkulptProducedOutputString = class extends CustomType {
  constructor(string2) {
    super();
    this.string = string2;
  }
};
var SkulptRanCode = class extends CustomType {
  constructor(outcome) {
    super();
    this.outcome = outcome;
  }
...
function default$() {
  let code2 = 'print("Hello, Python!")';
  return new Model2(code2, new None(), new None(), 0, 0);
}
function serialise(model) {
  let code2 = model.code;
  let times_edited = model.times_edited;
  let times_ran = model.times_ran;
  return to_string(times_edited) + "-" + (to_string(times_ran) + "-") + code2;
}
function deserialise(value3) {
  return try$(
    split_once(value3, "-"),
    (_use0) => {
      let times_edited = _use0[0];
      let rest = _use0[1];
      return try$(
        split_once(rest, "-"),
        (_use02) => {
          let times_ran = _use02[0];
          let code2 = _use02[1];
          return try$(
            parse_int(times_edited),
            (times_edited2) => {
              return try$(
                parse_int(times_ran),
                (times_ran2) => {
...
function code_mirror(attributes) {
  return element("code-mirror", attributes, toList([]));
}
function code_editor(model) {
  let class$2 = (() => {
    let $ = model.outcome;
    if ($ instanceof Some && $[0] instanceof Failure) {
      return class$("failure");
    } else if ($ instanceof Some && $[0] instanceof Success) {
  ...
  let output_text = (() => {
    let $ = model.output;
    if ($ instanceof Some && $[0] instanceof FromPreviousRun) {
      let output = $[0].output;
      return output;
    } else if ($ instanceof Some && $[0] instanceof FromCurrentRun) {
      let output = $[0].output;
      return output;
    } else {
      return "";
  ...
  return div(
    toList([class$("code"), class$2]),
    toList([
      code_mirror(
        toList([
          on_input((var0) => {
            return new UserTypedCode(var0);
          }),
          id("code-editor"),
          class$("code-editor"),
          value(model.code)
      ...
      (() => {
        if (output_text$1 === "") {
          ...
          let output = output_text$1;
          return div(
            toList([class$("code-result")]),
            toList([
              pre(
                toList([]),
                toList([code(toList([]), toList([text2(output)]))])
              ),
              none3()
            ])
          );
...
function controls() {
  return div(
    toList([class$("controls")]),
    toList([
      button(
        toList([on_click(new UserPressedRunButton())]),
        toList([text2("Run")])
      ),
      button(
        toList([on_click(new UserPressedSaveButton())]),
        toList([text2("Save")])
      )
    ])
  );
...
function page_footer() {
  return footer(
    toList([]),
    toList([
      text2("This website is made with "),
      a(
        toList([href("https://gleam.run")]),
        toList([text2("Gleam")])
      ),
      text2(" and the Python code runs in the browser thanks to "),
      a(
        toList([href("https://skulpt.org")]),
        toList([text2("Skulpt")])
      )
    ])
  );
...
function view(model) {
  return div(
    toList([class$("limit-max-width-and-center")]),
    toList([
      h1(toList([]), toList([text2("Python editor")])),
      code_editor(model),
      controls(),
      pre(toList([id("output")]), toList([])),
      div(toList([id("canvas")]), toList([])),
      page_footer()
    ])
  );
}
function run_code(code2) {
  return from(
    (dispatch) => {
      let on_output = (output) => {
        return dispatch(new SkulptProducedOutputString(output));
      };
      let _pipe = do_run_code(code2, on_output);
      let _pipe$1 = map_promise(_pipe, (_) => {
        return new Success();
      });
      let _pipe$2 = rescue(
        _pipe$1,
        (error) => {
          return new Failure(skulpt_error_to_string(error));
        }
      );
      let _pipe$3 = map_promise(
        _pipe$2,
        (var0) => {
          return new SkulptRanCode(var0);
        }
      );
...
function save_code(model) {
  return from(
    (_) => {
      let now$1 = now();
      let metadata = (() => {
        let _pipe = toList([model.times_edited, model.times_ran, now$1]);
        let _pipe$1 = map(_pipe, to_string);
        return join(_pipe$1, "-");
      })();
      tap(
        sha256_hash(metadata),
        (hash) => {
          let hash$1 = base16_encode(hash);
          let heading = (() => {
            let _pipe = toList([
          ...
          let content = heading + "\n\n" + model.code;
          return save_file("solution.txt", content);
        }
      );
      return void 0;
    }
...
function save_model(model) {
  return from(
    (_) => {
      return save_to_session_storage("model", serialise(model));
    }
  );
}
function update(model, msg) {
  if (msg instanceof UserPressedSaveButton) {
    return [model, save_code(model)];
  } else if (msg instanceof UserPressedRunButton) {
    let output = (() => {
      let $ = model.output;
      if ($ instanceof Some && $[0] instanceof FromCurrentRun) {
        let output2 = $[0].output;
        return new Some(new FromPreviousRun(output2));
      } else if ($ instanceof Some && $[0] instanceof FromPreviousRun) {
        let output2 = $[0].output;
        return new Some(new FromPreviousRun(output2));
      } else {
        return new None();
    ...
    let model$1 = (() => {
      ...
      return new Model2(
        ...
        output,
        _record.outcome,
        _record.times_edited,
        _record.times_ran
      );
    })();
    return [model$1, run_code(model$1.code)];
  } else if (msg instanceof UserTypedCode) {
    let code2 = msg.code;
    ...
    return [model$1, save_model(model$1)];
  } else if (msg instanceof SkulptProducedOutputString) {
    let string2 = msg.string;
    let output = (() => {
      let $ = model.output;
      if ($ instanceof Some && $[0] instanceof FromCurrentRun) {
        let output2 = $[0].output;
        return new FromCurrentRun(output2 + string2);
      } else if ($ instanceof None) {
        return new FromCurrentRun(string2);
      } else {
        return new FromCurrentRun(string2);
      }
    })();
...
function main() {
  let model = (() => {
    let _pipe = read_from_session_storage("model");
    let _pipe$1 = deserialise(_pipe);
